...
test("calculateRatings: cup uses avg monthly conversion with correct divisors", () => {
  // Тренер с садом и школой: все 4 месяца
  const rowA = {
    ...sampleRow(),
    "ФИО": "Тренер А",
    "Дети_сад_октябрь": "5",
    "Дети_школа_октябрь": "10",
    "Дети_сад_март": "5",
    "Дети_школа_март": "10",
    "Кубок_метеор_октябрь": "3",   // / 15 (все) = 0.2
    "Кубок_метеор_декабрь": "4",   // / 10 (школа) = 0.4
    "Кубок_метеор_февраль": "2",   // / 5 (сад) = 0.4
    "Кубок_метеор_апрель": "5",    // / 10 (школа) = 0.5
  };
  // Среднее: (0.2 + 0.4 + 0.4 + 0.5) / 4 = 0.375

  // Тренер без сада: 3 месяца (февраль пропускается)
  const rowB = {
    ...sampleRow(),
    "ФИО": "Тренер Б",
    "Дети_сад_октябрь": "0",
    ...
    "Дети_сад_март": "0",
    "Дети_школа_март": "10",
    "Кубок_метеор_октябрь": "2",   // / 10 (все) = 0.2
    "Кубок_метеор_декабрь": "3",   // / 10 (школа) = 0.3
    "Кубок_метеор_февраль": "0",   // пропуск, нет сада
    "Кубок_метеор_апрель": "4",    // / 10 (школа) = 0.4
  };
  // Среднее: (0.2 + 0.3 + 0.4) / 3 = 0.3

  const coaches = [rowA, rowB].map(rowToCoach);
  calculateRatings(coaches);

  // А лучший (0.375) = 5, Б (0.3) = 0.3/0.375*5 = 4.0
  assert.equal(coaches[0].rating.scores.cup, 5);
  assert.equal(coaches[1].rating.scores.cup, 4);
});
